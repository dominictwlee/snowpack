...
export async function install(
  ...
  for (const installSpecifier of allInstallSpecifiers) {
    ...
    const proxiedName = webModulesSubImportPath(targetName); // sometimes we need to sanitize webModule names, as in the case of tippy.js -> tippyjs
  ...
  for (const [assetName, assetLoc] of Object.entries(assetEntrypoints)) {
    const assetDest = `${destLoc}/${webModulesSubImportPath(assetName)}`;
